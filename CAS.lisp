...
(defun simplify (fun &optional ruleset)
	(cond
		((null ruleset) (let* ((mfun (infix_to_prefix fun)) (result))
			(setq result (number_simplify mfun))
			(setq result (symbol_simplify result simplification_rules))
			(setq result (number_simplify result))
			(prefix_to_nfix result)
		))
		(T (let* ((mfun (infix_to_prefix fun)) (result))
			(setq result (number_simplify mfun))
			(setq result (symbol_simplify result ruleset))
			(setq result (number_simplify result))
			(prefix_to_nfix result)
		))
	)
...
(defun d/dx (fun)
	(let* ((processed (simplify fun)) (result))
		(setq processed (infix_to_prefix processed))
		(setq result (d/d processed 'x))
		(if result
			(simplify (prefix_to_nfix result))
...
(defun antiderivative (fun d)
	(let* ((processed) (result))
		(setq processed (infix_to_prefix (simplify fun)))
